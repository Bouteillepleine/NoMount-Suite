...
### Added
- **Hidden paths card in the WebUI.** `nomount whiteout add/remove/list/suggest` has existed for a while with no way to reach it short of a root shell. There is now a card that lists each hidden path with its real state, adds and removes them, and offers the device-specific suggestions.
