...
Sources carry ONE local change, added by `7e4222e`: `readphdrtable()` in
`sstrip.c` seeks to `ehdr.e_phoff` before reading the program header table.
Upstream has no such seek. Keep it when refreshing - re-extracting the tarball
and copying the file over reverts it silently.

Only the subset `sstrip` links is kept: the rest of libelfrw
(`elfrw_dyn/rel/shdr/sym/ver`) is unreferenced by `sstrip.c`. The Makefile is
ours, replacing upstream's two-directory `libelfrw.a` build.
